...
**语言支持**：应用和文档均支持简体中文与英文。应用默认按系统首选语言顺序选择支持的语言，没有匹配时回退为英文。菜单中的 **语言 → 跟随系统 / 简体中文 / English** 可立即切换并保存偏好，帮助窗口也提供语言选择入口。会话标题、任务内容、项目路径和外部服务返回的信息保持原文。

## 界面与简短演示

![中文新手引导](docs/images/help-zh.png)

[English interface](docs/images/help-en.png) · [关于与更新](docs/images/about-zh.png)

![中英文界面演示](docs/images/bilingual-tour.gif)

以上为使用虚构数据渲染的原生应用界面。简短演示轮播中英文帮助和版本信息，不包含真实账户或会话内容。
...
状态栏菜单中的“自动刷新额度”可直接开启或关闭此功能，默认关闭。开启后，默认按本地时间每天 05:30, 10:30, 15:30, 20:30 发起一次最小化的本机 Codex CLI 请求；它与原有的额度读取进程相互独立，不会改变额度读取的 60 秒刷新频率。点击“修改触发时间…”可按 `HH:mm` 的 24 小时制设置一个或多个时间，统一使用英文逗号分隔，例如 `06:00, 12:30, 18:00`。中英文界面使用同一格式，不接受中文逗号、顿号或空白项；已保存的计划不受影响。保存后会立即重新安排下一次触发。
...
读取失败时保留上次额度，同时在菜单栏加上 `⚠` 并标记“已过期”；超过 3 分钟的数据也会标记过期。菜单同时显示数据年龄和完整更新时间。成功读取或收到实时更新后恢复“最新”状态，打开菜单时会更新这些提示。

## 自动刷新执行结果

菜单显示最近一次执行的时间、成功或失败、触发原因和下次计划时间。失败只显示安全类别，例如未找到 CLI、超时或额度用尽，不把原始错误写入设置。额度恢复后的重试时间单独列出；如果应用退出时仍在执行，下次启动会标记“上次刷新已中断”。

## 版本与更新

从菜单打开“关于与更新…”可查看当前发布版本、MIT 许可和 GitHub 发布页。仅点击“检查更新”时读取公开的 GitHub 发布列表，不携带 Codex 凭据或会话数据。预览版也会检查新的预览版，正式版只检查正式版。发现新版后进入对应发布页手动下载，不会自动安装。
